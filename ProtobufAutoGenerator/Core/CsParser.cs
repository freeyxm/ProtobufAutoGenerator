...
namespace ProtobufAutoGenerator.Core
{
    class CsParser
    {
        ...
        private static Regex m_fieldRegex = new Regex("\\s*((private|protected|public)\\s+){0,1}(static\\s+){0,1}(const\\s+){0,1}(readonly\\s+){0,1}([a-zA-Z0-9_<>\\.]+)\\s+([a-zA-Z0-9_]+)\\s*(=\\s*(\\S+|new\\s+\\S+)){0,1};\\s*(//.+){0,1}", RegexOptions.Compiled);
        ...
        private static Regex m_argRegex = new Regex("\\s*((ref|out)\\s+){0,1}([a-zA-Z0-9_<>\\.]+)\\s+([a-zA-Z0-9_]+)\\s*(=\\s*(\\S+|new\\s+\\S+)){0,1}", RegexOptions.Compiled);
